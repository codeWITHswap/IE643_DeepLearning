...
def mean_pooling(batch, lengths):
    ...
    for i, _ in enumerate(batch):
        if int(lengths[i])==0:
            lengths[i]+=1
        if i==0:
            tmp_vec=nn.functional.avg_pool1d(_.transpose(0, 1).unsqueeze(0), int(lengths[i]))[:, :, 0] #applies a 1D average pooling over an input signal composed of several input planes.
        else:
...
class BiGRU_centric_layer(nn.Module):
    '''
    basic BiGRU review-centric layer
    input:
        in_size - input_size for GRU
        hidden_size - hidden_size for GRU
    ...
    def __init__(self, in_size, hidden_size, num_heads=4, dropout_rate=0.2):
        super(BiGRU_centric_layer, self).__init__()
        self.in_size=in_size
        self.hidden_size=hidden_size
        self.num_heads=num_heads
        self.dropout_rate=dropout_rate
        self.dropout_layer=nn.Dropout(p=self.dropout_rate)
        self.gru_raw=nn.GRU(self.in_size, self.hidden_size, bidirectional=True, batch_first=True)
        self.gru_sum=nn.GRU(self.in_size, self.hidden_size, bidirectional=True, batch_first=True)
    ...
    def forward(self, 
                ...
                use_divide_dk=False):
        output_raw, (h_n_raw, c_n_raw)=self.gru_raw(in_raw)
        output_raw=self.dropout_layer(output_raw)
        if use_sum_gru:
            output_sum, (h_n_sum, c_n_sum)=self.gru_sum(in_sum)
...
class BiGRU_centric_model(nn.Module):
    '''
        BiGRU review-centric model
    '''
    def __init__(self, 
                in_size, 
                hid_size, 
                ...
                use_divide_dk=False):
        super(BiGRU_centric_model, self).__init__()
        self.in_size=in_size
        ...
        self.gru_sum=nn.GRU(self.in_size, self.hid_size, batch_first=True, bidirectional=True)
        self.attention_layers=nn.ModuleList([BiGRU_centric_layer(self.in_size, self.hid_size, dropout_rate=self.dropout_rate, num_heads=self.num_heads) for i in range(self.num_layers)])
    ...
    def forward(self, in_vec_raw, in_vec_sum,  len_raw, len_sum,  use_norm=False,  use_sum_avg_pooling=True):
        output_sum, (h_n, c_n)=self.gru_sum(in_vec_sum)
        output_sum=self.dropout_layer(output_sum)
